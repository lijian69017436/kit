...
if (window.location.host != 'fkgg07.faisco.cn') { window.location.href = 'http://' + 'fkgg07.faisco====.cn'; } 
	//设置浏览器 地址栏的地址
$(function(){ //表单提交的 时候 可以 做处理 
	...
	$(inputForm).form({
	    ...
	    success:function(data){//表单 提交 返回的 结果  
	    	var result = jQuery.parseJSON(data);//利用 jquery 解析  json 串 
	    	if(result.isok=='T'){
	    		...
	    		if(flag){
	    			if(flag==1){
	    				$('#unamespan').text(result.msg);
	    				$("#uname").focus();
	    			}
	    			if(flag==2){
	    				$('#upassspan').text(result.msg);
	    				$("#upass").focus();
	    			}
	    			if(flag==3){
	    				$('#randspan').text(result.msg);
	    				$("#randImge").focus();
	    			}
	    		}
	    		getImage();
...
///用法  在  jsp 引用 地下 的 js  直接 password.md5()  就行
////////////////////////////////////////////////////////////////底下 为 md5 加密算法 ///////////////////////////////////////////////////////////////////////
...
String.prototype.ub64=function ()
{
    var input=this;
    var keyStr = "ABCDEFGHIJKLMNOP" +	"QRSTUVWXYZabcdef" +	"ghijklmnopqrstuv" +	"wxyz0123456789+/" +	"=";
    var output = "";
