...
final class ToBeanResultTransformSafety implements ResultTransformer {
	...
	public Object transformTuple(Object[] tuple, String[] aliases) {
		...
		for (int i = 0; i < aliases.length; i++) {
			try {
				...
				if (filed != null) {
					if ((filed.getType() != null) && (filed.getType() == Integer.class)) {
	...
	private Field getFiledByColumnName(Field[] fields, String alias) {
		if ((fields == null) || (fields.length == 0)) {
			throw new RuntimeException(
					"[JAVA REFLECT ERROR] Cann't find any declared field in Class: " + this.resultClass.getName());
		}
